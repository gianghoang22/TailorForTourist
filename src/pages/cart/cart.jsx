...
import {
  getCart,
  removeFromCart as removeCustomProduct,
} from "../../utils/cartUtil";
...
const Cart = () => {
  ...
  const calculateTotal = () => {
    const customTotal = customCart.reduce(
      (total, item) => (item.price ? total + item.price : total),
      0
    );
    const nonCustomTotal = nonCustomCart.reduce(
      (total, item) => (item.price ? total + item.price : total),
      0
    );
                          ...
                          item.type === "SUIT" && (
                            <tr key={item.id}>
                              <td className="product-name">
                                SUIT - {item.fabric ? item.fabric.name : "N/A"}
                              </td>
                              ...
                              <td className="product-lining">
                                {item.lining ? item.lining.name : "N/A"}
                              </td>
